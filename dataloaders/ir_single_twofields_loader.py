...
@DatasetReader.register("irSingle2Fields")
class IrSingleTwofieldsDatasetReader(DatasetReader):
    """
    ...
    Expected format for each input line: <id>\t<sequence_string>

    The output of ``read`` is a list of ``Instance`` s with the fields:
        id: ``LabelField`` and
        sequence_tokens: ``TextField``
    ...
    def _read(self, file_path):
        with open(cached_path(file_path), "r", encoding="utf8") as data_file:
            ...
            for line_num, line in enumerate(data_file):
                ...
                sequenceid, text = line_parts
                if self.preprocess != None:
                    text = self.preprocess(text)
                yield self.text_to_instance(sequenceid, text)

    @overrides
    def text_to_instance(self, sequenceid: str, text: str) -> Instance:  # type: ignore
        # pylint: disable=arguments-differ
        
        id_field = MetadataField(sequenceid)
        
        if len(text) > MAX_TEXT_LENGTH:
            print ("ID %s has %d chars. Trimed to %d chars" % (sequenceid, len(text), MAX_TEXT_LENGTH))
        tokenized_text = self._tokenizer.tokenize(text[:MAX_TEXT_LENGTH])
        text_field = TextField(tokenized_text, self._token_indexers)

        
        return Instance({"id": id_field, "text_tokens": text_field})
