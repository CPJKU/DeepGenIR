...
class PointerGeneratorNetwork(Model):
    '''
        The PyTorch implementation of PGN for IR is based on
        the PGN model implemented in Summarus by Ilya Gusev:
    ...
    def forward(self,
        ...
        logprobs = self._forward_loop(state, target)
...
class PointerGeneratorNetworkVariation1(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 source_embedder: TextFieldEmbedder,
                 ...
                 projection_dim: int = None,
                 tie: bool = True) -> None:
        super(PointerGeneratorNetworkVariation1, self).__init__(vocab)
        ...
        self._transf_encoder = TransformerEncoder(encoder_layer, enc_transf_num_layers)
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # shape: (batch_size, max_input_sequence_length, encoder_output_dim)
        embedded_input_contextualized = self._transf_encoder(embedded_input.transpose(0, 1), src_key_padding_mask = ~util.get_text_field_mask(source_tokens).bool()).transpose(0, 1)
...
class PointerGeneratorNetworkVariation2(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 source_embedder: TextFieldEmbedder,
                 max_decoding_steps: int=20,
                 ...
                 projection_dim: int = None,
                 tie: bool = True) -> None:
        super(PointerGeneratorNetworkVariation2, self).__init__(vocab)
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        embedded_input_contextualized = self._transf_encoder(embedded_input, src_key_padding_mask = ~util.get_text_field_mask(source_tokens).bool()).transpose(0, 1)
